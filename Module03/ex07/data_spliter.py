...
def data_spliter(x, y, proportion, indexes=False):
    ...
    try:
        ...
        if len(y.shape) == 1:
            y = y.reshape(-1,1)
        data = np.array(np.append(x, y[:,-1].reshape(-1,1), axis=1))
        ...
        print("Error: data_spliter: {0}: {1}".format(type(err).__name__, err))
        return None
